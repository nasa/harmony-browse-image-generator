...
def create_browse_imagery(
    ...
    try:
        # open_rasterio returns Dataset | list[Dataset] if there are subdatasets in the
        # input_file_path or if band_as_variable=True (which is not used here)
        # Since we use the _manager private member which is only available for the DataArray
        # output, raise an error for any case where the opened file is unexpectedly not a
        # DataArray.
        xds = open_rasterio(input_file_path, mode="r", mask_and_scale=True)
        if isinstance(xds, DataArray):
            with xds as rio_in_array:
                ...
                for grid_parameters, tile_location in zip_longest(
                    ...
                    tiled_out_image_file = get_tiled_filename(
                        out_image_file, tile_location
                    )
                    tiled_out_world_file = get_tiled_filename(
                        out_world_file, tile_location
                    )
            ...
            raise HyBIGError(f"subdatasets are not supported, read in {type(xds)}")
...
def convert_mulitband_to_raster(data_array: DataArray) -> NDArray[np.uint8]:
...
def scale_grey_1band(data_array: DataArray) -> tuple[NDArray, ColorMap]:
    ...
    normalized_data = norm(band)
    normalized_data = cast(np.ma.MaskedArray, normalized_data)

    # if no color palette was provided, we need to determine the nodata value
    # from the raster.
    ndv_tuple: tuple[float | None, ...] = (None,)
    if data_array.rio._manager is not None:
        reader = data_array.rio._manager.acquire()
        ndv_tuple = reader.get_nodatavals() or (None,)

    # Scale input data from 0 to 254 if there is a nodata value, else 255
    if ndv_tuple[0] is not None:
        quantized_data = normalized_data * 254.0
        has_ndv = True
    else:
        quantized_data = normalized_data * 255.0
        has_ndv = False

    # Set any missing to missing. If there is no nodata value, this will
    # conflict with the highest quantization level, but that is acceptable
    # since the behavior of a nan value is undefined in such cases
    quantized_data[np.isnan(normalized_data)] = NODATA_IDX

    grey_colormap = greyscale_colormap(with_ndv=has_ndv)
    raster_data = np.expand_dims(np.round(quantized_data).data, 0)
...
def scale_paletted_1band(
    ...
    scaled_band = cast(np.ma.MaskedArray, scaled_band)
...
def get_color_map_from_image(image: ImageFile.ImageFile) -> dict:
    ...
    for idx, color_tuple in enumerate(color_tuples):
        color_map[np.uint8(idx)] = tuple(color_tuple)
    return color_map
